...
def make_chart(simulation: Dict[str, Any], chart_path: Path, title_date_end: str) -> None:
    ...
    def _series_in_billions(entry: Dict[str, Any]) -> List[float]:
        return [point['cumulative_loss_usd'] / 1_000_000_000 for point in sorted(entry['series'], key=lambda x: x['date'])]
    ...
    world_billions = _series_in_billions(simulation['world'])
    usa_billions = _series_in_billions(usa) if usa else []
    china_billions = _series_in_billions(china) if china else []
    ...
    ax.plot(dates, world_billions, linewidth=3.0, color='#98d08b', label='World')
    ax.fill_between(dates, world_billions, 0, color='#98d08b', alpha=0.14)

    if usa_billions:
        ax.plot(dates, usa_billions, linewidth=2.4, color='#60a5fa', label='United States')
    if china_billions:
        ax.plot(dates, china_billions, linewidth=2.4, color='#f59e0b', label='China')
    ...
    ax.set_title(
        f'Simulated Cumulative Blackout Loss - World, US, China: {dates[0].isoformat()} to {title_date_end} UTC',
        color='#e5e7eb',
        fontsize=18,
        pad=22,
    )
    ...
    legend = ax.legend(loc='upper left', facecolor='#0b0f14', edgecolor='none', framealpha=0.0, fontsize=11)
    ...
    fig.text(0.66, 0.09, 'World', color='#98d08b', fontsize=11, fontweight='bold')
    fig.text(0.715, 0.09, fmt_money(world_series[-1]['cumulative_loss_usd']) if world_series else '$0', color='#e5e7eb', fontsize=11)
    if usa:
        fig.text(0.66, 0.06, 'US', color='#60a5fa', fontsize=11, fontweight='bold')
        fig.text(0.69, 0.06, fmt_money(usa['cumulative_loss_usd']), color='#e5e7eb', fontsize=11)
    if china:
        fig.text(0.83, 0.06, 'China', color='#f59e0b', fontsize=11, fontweight='bold')
        fig.text(0.89, 0.06, fmt_money(china['cumulative_loss_usd']), color='#e5e7eb', fontsize=11)
    ...
    return f"""
        <section class="interactive-chart-block">
          <div class="chart-header">
            <h2>Interactive cumulative loss chart: World, United States, China</h2>
            <p>Move the cursor over the chart to compare the cumulative simulated losses for the world scenario, the United States, and China on the same date.</p>
          </div>
          <div class="chart-summary" id="{summary_id}">Day {post['simulation']['reference_days']} · {post['date']} · World {fmt_money(post['simulation']['world']['cumulative_loss_usd'])} · US {fmt_money(usa_now['cumulative_loss_usd'])} · China {fmt_money(china_now['cumulative_loss_usd'])}</div>
          ...
          const chart = new Chart(canvas, {{
            ...
            options: {{
                ...
                tooltip: {{
                  ...
                  callbacks: {{
                    ...
                    label: (context) => {{
                      ...
                      if (context.dataset.label === 'World') {{
                        return 'World · Day ' + point.day + ' · ' + formatUSD(point.world_cumulative_usd);
                      }}
                      if (context.dataset.label === 'United States') {{
              ...
              onHover: (_event, activeEls) => {{
                ...
                updateSummary(chartPoints[index]);
              }}
            }}
          }});
